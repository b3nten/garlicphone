...
func (it *Player) fromBytes(data []byte, fieldIndex uint16, offset int) (int, error) {
	switch fieldIndex {
	case 0: val, len, err := newListDeserializer[[]Foo](newListDeserializer[Foo](deserializeStruct[Foo]))(data, offset); it.Nested = &val; return len, err
	case 1: val, len, err := deserializeUint32(data, offset); it.Id = &val; return len, err
	case 2: val, len, err := deserializeString(data, offset); it.Name = &val; return len, err
	case 3: val, len, err := newListDeserializer[Foo](deserializeStruct[Foo])(data, offset); it.Inventory = &val; return len, err
	case 4: val, len, err := deserializeStruct[Foo](data, offset); it.Idk = &val; return len, err
	}
	return 0, UnknownFieldError
}
